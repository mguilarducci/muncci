...
var ApplicationConfiguration = function () {
  var applicationModuleName = "muncci", applicationModuleVendorDependencies = ["ngResource", "ngCookies", "ngAnimate", "ngTouch", "ngSanitize", "ui.router", "ui.bootstrap", "ui.utils"], registerModule = function (moduleName, dependencies) {
    angular.module(moduleName, dependencies || []), angular.module(applicationModuleName).requires.push(moduleName)
  };
  return{applicationModuleName: applicationModuleName, applicationModuleVendorDependencies: applicationModuleVendorDependencies, registerModule: registerModule}
}();
...
}), ApplicationConfiguration.registerModule("core"), ApplicationConfiguration.registerModule("payables"), ApplicationConfiguration.registerModule("transactions"), ApplicationConfiguration.registerModule("users"), angular.module("core").config(["$stateProvider", "$urlRouterProvider", function ($stateProvider, $urlRouterProvider) {
  $urlRouterProvider.otherwise("/"), $stateProvider.state("home", {url: "/", templateUrl: "modules/core/views/home.client.view.html"})
...
}]), angular.module("payables").run(["Menus", function (Menus) {
  Menus.addMenuItem("topbar", "Payables", "payables", "dropdown", "/payables/"), Menus.addSubMenuItem("topbar", "payables", "List payables", "payables")
}]), angular.module("payables").config(["$stateProvider", function ($stateProvider) {
  $stateProvider.state("listPayables", {url: "/payables", templateUrl: "modules/payables/views/list-payables.client.view.html"})
}]), angular.module("payables").controller("PayablesController", ["$scope", "$stateParams", "$location", "Authentication", "Payables", function ($scope, $stateParams, $location, Authentication, Payables) {
  $scope.authentication = Authentication, $scope.find = function () {
    $scope.payables = Payables.query(), console.log($scope.payables)
  }, $scope.rowClass = function (payable) {
    return payable.total > 0 ? "list-group-item-success" : "list-group-item-danger"
  }
}]), angular.module("payables").factory("Payables", ["$resource", function ($resource) {
  return $resource("payables/:payableId", {transactionId: "@_id"}, {update: {method: "PUT"}})
}]), angular.module("transactions").run(["Menus", function (Menus) {
  Menus.addMenuItem("topbar", "Transactions", "transactions", "dropdown", "/transactions(/create)?"), Menus.addSubMenuItem("topbar", "transactions", "List Transactions", "transactions"), Menus.addSubMenuItem("topbar", "transactions", "New Transaction", "transactions/create")
}]), angular.module("transactions").config(["$stateProvider", function ($stateProvider) {
  $stateProvider.state("listTransactions", {url: "/transactions", templateUrl: "modules/transactions/views/list-transactions.client.view.html"}).state("createTransaction", {url: "/transactions/create", templateUrl: "modules/transactions/views/create-transaction.client.view.html"}).state("viewTransaction", {url: "/transactions/:transactionId", templateUrl: "modules/transactions/views/view-transaction.client.view.html"}).state("editTransaction", {url: "/transactions/:transactionId/edit", templateUrl: "modules/transactions/views/edit-transaction.client.view.html"})
}]), angular.module("transactions").controller("TransactionsController", ["$scope", "$stateParams", "$location", "Authentication", "Transactions", function ($scope, $stateParams, $location, Authentication, Transactions) {
  $scope.authentication = Authentication, $scope.create = function () {
    var transaction = new Transactions({name: this.name, to: this.to, kind: this.kind, status: this.status, value: this.value, date: this.date, dueDate: this.dueDate});
    transaction.$save(function (response) {
      $location.path("transactions/" + response._id), $scope.name = "", $scope.to = "", $scope.kind = "", $scope.status = "", $scope.value = "", $scope.date = "", $scope.dueDate = ""
    ...
    if (transaction) {
      transaction.$remove();
      for (var i in $scope.transactions)$scope.transactions[i] === transaction && $scope.transactions.splice(i, 1)
    } else $scope.transaction.$remove(function () {
    ...
    var to, kind, fromTo;
    return transaction.user._id === user._id ? ("pay" === transaction.kind ? (kind = "Pagar", fromTo = "a") : (kind = "Receber", fromTo = "de"), to = transaction.friend ? transaction.friend.displayName || transaction.to : transaction.to) : ("pay" === transaction.kind ? (kind = "Receber", fromTo = "de") : (kind = "Pagar", fromTo = "a"), to = transaction.user.displayName), kind + " R$" + transaction.value + " " + fromTo + " " + to
  }, $scope.canRevoke = function (transaction, user) {
    return transaction.user._id !== user._id && transaction.to === user.email && "created" === transaction.status
  }, $scope.canAccept = function (transaction, user) {
    return transaction.user._id !== user._id && transaction.to === user.email && ("created" === transaction.status || "revoked" === transaction.status)
  }, $scope.canPay = function (transaction, user) {
    return!(transaction.user._id !== user._id && transaction.to !== user.email || "created" !== transaction.status && "accepted" !== transaction.status)
  }, $scope.revoke = function (transaction) {
    transaction && ($scope.transaction = transaction), $scope.transaction.status = "revoked", $scope.update()
  }, $scope.accept = function (transaction) {
    transaction && ($scope.transaction = transaction), $scope.transaction.status = "accepted", $scope.update()
  }, $scope.pay = function (transaction) {
    transaction && ($scope.transaction = transaction), $scope.transaction.status = "paid", $scope.update()
...
}]), angular.module("transactions").factory("Transactions", ["$resource", function ($resource) {
